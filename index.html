	  ...
	  <div id="treemap-legend" class="flex-container" style="flex-direction: row; justify-content: space-between; padding-bottom: 5px; order: 5; flex: 0 1 auto;">
		...
		<div style="text-align: right; width=10%; ">
		  <ul style="list-style: none; margin-top: 0;">
		    <li style="color: #bbb;">NGA/TAED</span>
		    <li style="color: #bbb;">Treemap v01May20</span>
		  </ul>
		</div>
      </div>
    </div>
...
var CSVfileName = "https://mdbaehre.github.io/CNCFLandscape.csv"  //the name of csv file (including its path relative to the location of the html file)
...
var leafNodeSizeValueColumns = ["value"]; //the column name in the CSV file that holds numerical values used to determine the leaf node size
...
d3.csv(CSVfileName).then(function(data){ //name of csv file to use
  qcConfigurationData(data);
  let initialGroupingOrder = groupingOrderSet[Object.keys(groupingOrderSet)[0]].slice();
...
function createNodeColorLegend() {
  ...
  if (colorProperty != "Default color"){
	...
	if (leafNodeSizeValueColumns.includes(colorProperty)){
	  colorSet = d3.schemeRdYlGn[pv.length]; //use a heatmap range of red-yellow-green for numerical values
	}
    for (let i = 0; i < pv.length; i++){
      if (pv[i] != ""){
        domString += '<li><input type="color" id="'+pv[i]+'" value="'+colorSet[i]+'" onchange="changeLeafNodeColor(this)" /><label for="'+pv[i]+'">'+ truncate(pv[i]) + '</label></li>';
	  }else{
	    domString += '<li><input type="color" id="'+pv[i]+'" value="'+colorSet[i]+'" onchange="changeLeafNodeColor(this)" /><label for="'+pv[i]+'">(Blanks)</label></li>';
	  }
	  leafNodeColor[pv[i]] = colorSet[i]; //build leafNodeColor array used to color each leaf node
	}
